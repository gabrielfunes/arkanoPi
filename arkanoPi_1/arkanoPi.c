...
int ConfiguraInicializaSistema (TipoSistema *p_sistema) {
	...
	//tmr_startms(sistema.arkanoPi.tmr_actualizacion_juego, TIMEOUT);
...
PI_THREAD (thread_explora_teclado_PC) {
	...
	while(1) {
		...
		p_teclado = (TipoTeclado*)(teclado_TL04.);

		ProcesaTeclaPulsada(p_teclado);
		*/
...
int main () {
	...
	fsm_trans_t interruptor_tmr[] = {
		{ APAGADO, CompruebaBotonPulsado, ENCENDIDO, encender },
		{ ENCENDIDO, boton_pulsado, ENCENDIDO, encender },
		{ ENCENDIDO, timeout, APAGADO, apagar },
		{-1, NULL, -1, NULL },
	};

	fsm_t* interruptor_tmr_fsm = fsm_new (APAGADO, interruptor_tmr, tmr);
	...
	fsm_t* arkanoPi_fsm = fsm_new (WAIT_START, arkanoPi, &sistema);  //SE DEBERÍA CAMBIAR A ARKANOPI


	//Maquina estados teclado
	...
	//Maquina de estados display LEDS
	...
	/*
	fsm_destroy (fsm_trans_excitacion_columnas);
	fsm_destroy (fsm_trans_deteccion_pulsaciones);
	fsm_destroy (fsm_trans_excitacion_display);
	*/
}
